...
pub enum Error {
    ...
    /// A row of the bundled autonomous-system data could not be read.
    ///
    /// Loud on purpose. The bundle is generated by a documented command, and a row this
    /// parser cannot read means that command was changed or half-ran; loading the rest and
    /// carrying on would mis-name somebody's provider without ever saying so.
    #[error("{file}: line {line} is not a valid record (`{raw}`)")]
    MalformedAsnRow {
        /// Which of the two bundled files the row came from.
        file: &'static str,
        /// Line number within that file, counted from one.
        line: usize,
        /// The rejected row, truncated, kept for diagnostics.
        raw: String,
    },

    /// The autonomous-system names outgrew the offsets that index them.
    ///
    /// Needs four gigabytes of registered names to reach; it exists so that building the
    /// table has no failing case that would have to panic.
    #[error("the autonomous-system name table is too large to index")]
    AsnDataTooLarge,
